...
export default function Home() {
    ...
    const { data: stats, isLoading: isStatsLoading } = api.user.getStats.useQuery({ userId: user?.id }, { enabled: !!user?.id });
    const { data: athletes = [], isLoading: isAthletesLoading } = api.user.getAllAthletes.useQuery({ idUser: user?.id ?? "" }, { enabled: !!user?.id });
    ...
    return (
        <div className="w-full max-w-6xl mx-auto p-4 sm:p-6 lg:p-10 space-y-8 animate-in fade-in duration-500">
            <div className="flex flex-col gap-1">
                ...
                <p className="text-zinc-500 font-medium text-sm sm:text-base">Panoramica operativa del portale Black Bulls Volley.</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <Card className="rounded-3xl border border-zinc-200 shadow-sm bg-white p-6">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <p className="text-xs font-bold text-zinc-400 uppercase tracking-widest">Atleti Associati</p>
            ...
            <div className="space-y-4">
                <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs font-bold text-zinc-400 uppercase tracking-wider">{athletes.length} Registrati</span>
                </div>
                                ...
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-3.5 min-w-0">
                                        ...
                                        <div className="min-w-0">
                                            ...
                                            <p className="text-xs text-zinc-400 font-semibold uppercase tracking-wider mt-0.5">Scheda Atleta</p>
                                        </div>
                                    </div>
