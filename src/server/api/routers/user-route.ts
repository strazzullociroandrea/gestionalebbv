                ...
                try {
                    ...
                    if (expirationMedicalCertificate !== undefined) {
                        ...
                        await ctx.db.delete(Notification)
                            .where(
                                and(
                                    eq(Notification.idAthlete, idAthlete),
                                    eq(Notification.text, "Scadenza certificato medico")
                                )
                            );
                        if (expirationMedicalCertificate && expirationMedicalCertificate.trim() !== "") {
                            await ctx.db.insert(Notification)
                                .values({
                                    id: crypto.randomUUID(),
                                    dateCreation: new Date().toISOString(),
                                    dateExpiration: expirationMedicalCertificate,
                    ...
                    if (expiredCI !== undefined) {
                        updatePayload.expiredCI = expiredCI?.toUpperCase();

                        await ctx.db.delete(Notification)
                            .where(
                                and(
                                    eq(Notification.idAthlete, idAthlete),
                                    eq(Notification.text, "Scadenza carta di identità")
                                )
                            );
                        if (expiredCI && expiredCI.trim() !== "") {
                            await ctx.db.insert(Notification)
                                .values({
                                    id: crypto.randomUUID(),
                                    dateCreation: new Date().toISOString(),
                                    dateExpiration: expiredCI,
